...
void flEnd(long endPlay[],int nPlayer,bool end,long round);
    ...
    do
    {
        ...
        else if (!jailP[turn])
        {
            do //Players can roll doubles twice in a turn without penalty
            {
                ...
                {
                    cout<<prop<<" is owned by "<<p[ownedBy]<<".\n";
                ...
                else if (ownedBy==100) //If the purchaseable property is not currently owned
                {
                    ...
                    {
                        if (moneyP[turn]>propCst[0][pos-1]) //They purchase the property
                        {
                            owner[pos-1]=turn;
                            moneyP[turn]-=propCst[0][pos-1];
                            cout<<p[turn]<<" has purchased "<<prop<<".\n"<<endl;
                            chkBank(p,moneyP,turn);
                        }
                        else //They do not purchase the property
                            cout<<p[turn]<<" doesn't have enough cash to purchase"
                                <<prop<<".\n"<<endl;
                    }
                }
